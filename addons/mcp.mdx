...
description: Connect AI assistants like Claude and Cursor to your TREK data via the Model Context Protocol.
---
...
The MCP addon must be enabled by your TREK administrator before any user can access it. Until it is enabled, the `/mcp` endpoint returns `403 Forbidden` and the MCP section does not appear in user settings.

## Setup

<Steps>
  <Step title="Enable the MCP addon">
    An administrator must enable the MCP addon from **Admin Panel → Addons**. This makes the `/mcp` endpoint available and exposes the MCP configuration section in user settings.
  </Step>
  <Step title="Create an API token">
    Go to **Settings → MCP Configuration** and create an API token:
    ...
    <Warning>
      The token is shown only once and cannot be recovered. If you lose it, delete the token and create a new one.
    </Warning>

    Each user can create up to **10 tokens**.
  </Step>
  <Step title="Configure your MCP client">
    The Settings page shows a ready-to-copy configuration snippet. For **Claude Desktop**, add the following to your `claude_desktop_config.json`:
    ...
    <Note>
      The path to `npx` may need to be adjusted for your system. On Windows, use `C:\PROGRA~1\nodejs\npx.cmd`.
    </Note>
  </Step>
</Steps>
...
| `create_trip` | Create a new trip with title, dates, and currency. Days are auto-generated from the date range. |
...
| Tool | Description |
|---|---|
| `create_place` | Add a place/POI with name, coordinates, address, category, notes, website, and phone. |
...
Exceeding the rate limit returns a `429` response. Exceeding the session limit prevents new sessions from connecting until an existing one expires or is terminated by deleting its token.
